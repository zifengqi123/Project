...
typedef pointer				proxy_handle;				//	代理句柄
...
typedef enum _FRAME_DATA_TYPE
{
	FDT_P_FREAM = 0,
	FDT_I_FRAME = 0x01,
	FDT_PARAM_CHANGED = 0x03,
	FDT_AUDIO_PCM = 0x05,
	FDT_AUDIO_SPEEX = 0x06,
	FDT_AUDIO_AAC = 0x07,
	FDT_AUDIO_AMR = 0x08,
	FDT_INFO_FRAME = 0x09,
	FDT_AUDIO_ADPCM = 0x0A,
	FDT_AUDIO_G711_ALAW = 0x0B,
	FDT_AUDIO_G711_ULAW = 0x0C
}FRAME_DATA_TYPE;
...
typedef struct _FRAME_INFO
{
	...
	uint16	frame_type;			//	帧类型 0-P帧 1-I帧 3-参数变化帧 5-PCM音频帧 6-SPEEX音频帧 7-AAC音频帧 9-信息帧
...
typedef struct _DEVICE_INFO
{
	...
	P_CHANNEL_CAPACITY  channel_capacity[20];	//	DVS通道能力，单个设备（对应1个通道，通道号为0）
...
typedef struct _VIDEO_RECORD_FILE_HEAD
{
    char	magic_num[32];
    uint32	head_size;
    uint32	video_fmt;
    ...
    char	dev_sn[68];
    char	dev_name[68];
} VIDEO_RECORD_FILE_HEAD, *P_VIDEO_RECORD_FILE_HEAD;
...
//	透传代理
typedef void (*pfn_on_proxy_resp)(user_data userdata, void* buf, int len);
HMCAPI hm_result hm_pu_proxy_open(user_id id, pfn_on_proxy_resp cb, void* userdata, void* req, int len, proxy_handle* handle);
HMCAPI hm_result hm_pu_proxy_send(proxy_handle handle, void* req, int len);
HMCAPI hm_result hm_pu_proxy_close(proxy_handle handle);
...
HMCAPI hm_result hm_pu_common_command_ex(user_id id, int32 cmdid, cpchar cmdbody, int32 bodylen, cpchar* recv_buf, int32* buf_len);
...
HMCAPI hm_result hm_util_push_service_uninit(push_service_handle handle);
    ...
    extern bool hm_sdk_local_playback_init(char* path, P_VIDEO_RECORD_FILE_HEAD* header);
    extern void hm_sdk_local_playback_uninit();
    extern void hm_sdk_local_playback_set_callback(cb_util_playback cb, user_data data);
    extern void hm_sdk_local_playback_start();
    extern void hm_sdk_local_playback_stop();
    extern void hm_sdk_local_playback_pause();
    extern void hm_sdk_local_playback_resume();
    extern real64 hm_sdk_local_playback_get_position();
    extern void hm_sdk_local_playback_set_position(real64 position);
