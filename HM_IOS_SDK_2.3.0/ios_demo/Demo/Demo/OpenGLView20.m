...
@property (assign, nonatomic) CGRect centalTouchRect;
...
- (void)displayYUV420pData:(void *)data width:(NSInteger)w height:(NSInteger)h
{
    ...
    if (_snapFlag)
    {
        unsigned char *rgbBuffer = (unsigned char *)malloc(w*h*4);
        memset(rgbBuffer, 0, w*h*4);
        
        Convert2RGB32(rgbBuffer, (unsigned char *)data, (unsigned char *)data + w * h, ((unsigned char *)data + w * h * 5 / 4),
                      h, w, w - w, w>>1, (w/2) - (w>>1));
    ...
    @synchronized(self)
    {
        ...
        glBindTexture(GL_TEXTURE_2D, _textureYUV[TEXU]);
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, w/2, h/2, GL_RED_EXT, GL_UNSIGNED_BYTE, (unsigned char *)data + w * h);
        
        // [self debugGlError];
        
        glBindTexture(GL_TEXTURE_2D, _textureYUV[TEXV]);
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, w/2, h/2, GL_RED_EXT, GL_UNSIGNED_BYTE, (unsigned char *)data + w * h * 5 / 4);
...
- (void)setVideoSize:(GLuint)width height:(GLuint)height
{
    ...
    glBindTexture(GL_TEXTURE_2D, _textureYUV[TEXU]);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RED_EXT, width/2, height/2, 0, GL_RED_EXT, GL_UNSIGNED_BYTE, (unsigned char *)blackData + width * height);
    
    glBindTexture(GL_TEXTURE_2D, _textureYUV[TEXV]);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RED_EXT, width/2, height/2, 0, GL_RED_EXT, GL_UNSIGNED_BYTE, (unsigned char *)blackData + width * height * 5 / 4);
